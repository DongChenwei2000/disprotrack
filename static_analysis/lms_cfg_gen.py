...
parser.add_argument('--exe', dest='exe', nargs='+', default="C:/ImportantFiles/First/course/Distributed_Computing/code/DisProTrack/binaries/openvpn", help='Provide the executable file location')
parser.parse_args([])
...
proj = angr.Project(args.exe,load_options={'auto_load_libs':False})
##################################################################################################
def log_functions():
  check_func = set()
  not_check_func = set()
